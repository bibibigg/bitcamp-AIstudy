...
#                                             # 'sigmoid'로 해줘야 한다.
# model.save('./_save/tf_15_cancer.h5')

model = load_model('./_save/tf_15_cancer.h5')
# 3. 컴파일, 훈련
model.compile(loss='binary_crossentropy', optimizer='adam',
              metrics=['accuracy', 'mse'])
from keras.callbacks import EarlyStopping
earlyStopping = EarlyStopping(monitor='var_loss', patience=50, mode='min',
                              restore_best_weights=True, verbose=1)

start_time = time.time()
hist = model.fit(x_train, y_train, epochs=5000, batch_size=200,
          validation_split=0.2, verbose=1,
          callbacks=[earlyStopping]) #verbose=0을 하면 훈련과정을 안볼 수 있음
...
#==============================================#
# patience=100
# loss :  0.9833208918571472
# acc :  0.8245614171028137
# 걸린 시간 :  3.2689106464385986
